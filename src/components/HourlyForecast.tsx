...
export default function HourlyForecast({ hourly }: HourlyForecastProps) {
  ...
  return (
    <div className="flex gap-0 overflow-x-auto pb-2 -mx-1 scrollbar-none">
      {items.map((time, i) => {
        const idx = start + i;
        ...
        return (
          <div
            key={time}
            className="flex flex-col items-center shrink-0 px-3 py-2 min-w-[52px] animate-fade-in-up"
            style={{ animationDelay: `${i * 30}ms` }}
          >
            <span className="text-[10px] text-muted-foreground font-mono">{i === 0 ? "Mnt" : hour}</span>
            <span className="text-sm font-mono mt-2 text-foreground">{temp}°</span>
            {precip > 0 && (
              <span className="text-[10px] text-pastel-blue-text font-mono mt-1">{precip.toFixed(1)}</span>
            )}
          </div>
        );
      })}
