          ...
          <Tooltip
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-background border border-border p-2 rounded shadow-sm text-xs flex flex-col gap-1">
                    <span className="text-muted-foreground font-mono">{d.time}</span>
                    <span className="font-mono text-foreground font-medium">{d.temp}°C</span>
                    {d.precip > 0 && (
                      <span className="text-pastel-blue-text font-mono">{d.precip.toFixed(1)} mm</span>
                    )}
                  </div>
                );
              }
